...
- **실용적인 기능 세트**:
  - **Excel 호환 복사/붙여넣기**: 탭 구분(TSV) 방식 지원 및 다중 행 자동 채우기.
  - **실행 취소(Undo)**: 최근 변경 사항에 대한 안정적인 복구.
  - **컨텍스트 메뉴**: 우클릭을 통한 주요 기능 접근.
- **고급 레이아웃**:
  - **틀 고정(Pinned Columns)**: 중요 열 좌/우 고정 기능.
  - **동적 열 조정**: 마우스 드래그를 이용한 너비 조절 및 정렬.
- **커스터마이징**:
  - **CSS 변수 기반 테마**: 손쉬운 색상 및 스타일 변경.
  - **유연한 서식**: 데이터 타입별 자동 포맷팅 및 사용자 정의 스타일.
...
const grid = new CKGrid({
    ...
    columns: [
        ...
        { 
            ...
            validator: (val) => !isNaN(val) || '숫자만 입력 가능합니다.'
        }
    ],
    data: myLargeData,
    pagination: true,
